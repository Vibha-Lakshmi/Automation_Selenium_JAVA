...
public class Demo {
	...
	void phoneBookSearchResult() throws InterruptedException, Exception {
		...
		/*
		 * Boolean isPresent =
		 * driver.findElements(By.xpath("//a[contains(text(),'Total records')]")).size()
		 * > 0; System.out.println("ispresent = "+isPresent);
		 */
		 ...
		 * // total record
		 * 
		 * if (isPresent) { System.out.println("in IF");
		 * 
		 * WebElement records;
		 * 
		 ...
		 * 
		 * records =
		 * driver.findElement(By.xpath("//a[contains(text(),'Total records')]"));
		 * 
		 * String num_records = records.getText(); System.out.println(num_records);
		 * String[] str = num_records.split(": "); int x = Integer.parseInt(str[1]);
		 * System.out.println("x = " + x); size_emp = x; } else { size_emp =
		 * names.size(); }
		 * 
		 * 
		 * List<WebElement> names =
		 ...
		 * 
		 * FileOutputStream fout = new FileOutputStream(".\\PrintData\\BloodGroup_" +
		 * blood_grp + ".txt"); BufferedOutputStream bout = new
		 * BufferedOutputStream(fout);
		 * 
		 * BufferedOutputStream bout = new BufferedOutputStream( new GZIPOutputStream(
		 * new FileOutputStream("D:\\BloodGroup.txt", true) ) );
		 * 
		 * for (int j = 0; j < size_emp; j++) { String emp_names = String.valueOf((j +
		 * 1)) + ". " + names.get(i).getText() + "\n"; System.out.println(emp_names);
		 * 
		 * // storing in file byte b[] = emp_names.getBytes(); bout.write(b);
		 * 
		 * i++; if ((i % 10) == 0) { i = 0; setElement(); } }
		 * 
		 * bout.flush(); bout.close(); // fout.close(); System.out.println("success");
		 * 
		 * WebElement back_btn; back_btn =
		 * driver.findElement(By.xpath("//span[contains(text(),'Back To Apps')]"));
		 * back_btn.click(); Thread.sleep(2000);
		 */
	}
